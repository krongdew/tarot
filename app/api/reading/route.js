...
console.log("Category received:", category);
  ...
  ไพ่ที่ลูกค้าเลือก:
  1. (สถานการณ์ปัจจุบันของผู้ถาม): ${cards[0].name} (${cards[0].nameTh})
  2. (อุปสรรคหรือสิ่งท้าทายที่กำลังเผชิญ): ${cards[1].name} (${cards[1].nameTh}) 
  3. (คำแนะนำหรือแนวทางแก้ไขสำหรับคำถามนี้): ${cards[2].name} (${cards[2].nameTh})
  4. (ผลลัพธ์หรือแนวโน้มในอนาคตหากทำตามคำแนะนำ): ${cards[3].name} (${cards[3].nameTh})
  
  ให้ทำการวิเคราะห์ความหมายของไพ่แต่ละใบโดยเชื่อมโยงกับคำถามอย่างตรงประเด็น
  คุณมีความเชี่ยวชาญในการตีความความสัมพันธ์ระหว่างไพ่และสามารถให้คำแนะนำที่เป็นประโยชน์ได้
  ...
  ตอบเป็น JSON ตามรูปแบบนี้:
  {
    ...
    "summary": "สรุปภาพรวมของไพ่ทั้ง 4 ใบและเชื่อมโยงกับคำถามโดยตรง แต่ต้องกระชับ",
    "advice": "คำแนะนำที่เป็นรูปธรรม ปฏิบัติได้จริง และตรงกับคำถามของลูกค้า"
  }
  
  สำคัญมาก:
  1. การตีความต้องตรงกับคำถามและหมวดหมู่ของคำถาม
  2. ให้คำอธิบายที่มีความหมายและเข้าใจง่าย
  3. ตอบตรงประเด็นและเฉพาะเจาะจงกับคำถามที่ถาม
...
const completion = await openai.chat.completions.create({
  ...
  temperature: 0.6, // เพิ่ม temperature ให้มีความคิดสร้างสรรค์มากขึ้น
  response_format: { type: "json_object" },
  max_tokens: 2500, // เพิ่ม token เพื่อให้ได้คำตอบที่ละเอียดขึ้น
  headers: headers
});
